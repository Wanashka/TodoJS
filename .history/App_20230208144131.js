...
function createTodo() {
  ...
  if (text === '') {
    ...
    page = Math.ceil(arrTodo.length / rows);
    filtration(arrTodo);
    inputTodo.value = '';
  }
}
...
function deleteAllCompleted() {
  const arrTodoDelete = arrTodo.filter((item) => item.checked !== true);
  // if (arrTodo.length === arrTodoDelete.length && checkboxAll.checked) {
  //   checkboxAll.checked = !checkboxAll.checked;
  // }
  filtration(arrTodoDelete);
}
