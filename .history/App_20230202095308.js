...
function editTask(event){
    ...
    if(event.target.classList.contains("input-todo")){
        if(item.id === taskId){
    todo.querySelector(".input-todo").textContent.hidden = true
    //todo.querySelector().createElement()
    const inputTask = createElement("input")
    todo.querySelector(".input-todo").appendChild(inputTask)
    //console.log(todo.querySelector(".input-todo").)
    }
    }
})
}
